/**
* @MaicQCR: 解析与应用拓展名为 *.qcr 的 MaicQCR 结构化存储文件
* @documentation: Markdown > hubclub(vscode)
* @author: MaicText Team
* @version: 2025.6.21
* 
* 
* 
* @description:
* * 默认解析的文件编码为 UTF-8，命名结构化规范追随 MaicQCR.Workbench，存储结构化规范默认为 CustomCFG beta。
* * 此头文件解析文件默认缺省为: `*.cfg1.is.qcr`
* * 此功能并非为集群消息消费者消费设定，只是作为本地记录的用途，因此在转发时需要保留额外备份。
* 
* 
* 
* @ADDON: MaicQCR.Workbench
* - 文件后缀名： `.qcr` 标明是 MaicQCR 根文件
* - 拓展命名规范遵守：`[Private.*].[Gerenal.*].[public.*].qcr`
* - 指导参考：https://github.com/ZoMaii/ZoMaii/blob/main/%5BBackup%5D%20license/workbench.md
* 
* 
* 
* @ADDON: CustomCFG beta
* - 注释行以符号 `#` 开头
* - 转义符号为 `\`，可用于转义注释符号或其他特殊字符
* - 赋值符为 `:`，前方为键名，后方为值
* - 寻址默认使用 Unix/Linux 风格的路径分隔符 `/`
* 
* 
* @USE
* 步骤：
* 1. 为单个文件创建操作对象，没有默认创建，默认 rw 权限。 （file_lock_create）
* 2. 锁定文件防止编辑期间被修改。（file_lock）
* 3. 读入文件内容到 _缓存区域,ORM工作区_ 进行引用对象进行 增删查改 操作 （file_read_content -> config_init > config_parse）
* 4. 提交缓存修改，确认后写入文件。(config_add_pair、config_update_pair、config_delete_pair)
* 5. 解除锁定，自动删除操作对象。( file_write_content(or new) > file_unlock > config_free)
*/
#pragma cfg1
